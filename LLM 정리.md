...
Transformer 아키텍처는 encoders와 decoders라는 두 부분으로 구성됩니다. Encoders와 decoders는 몇 가지 차이점을 제외하고는 거의 동일한 구조를 가지고 있습니다. (이에 대한 자세한 내용은 Transformer 아키텍처 입문서에서 확인하세요.) 또한 인코더와 디코더 스택의 장단점은 Autoregressive vs. Autoencoder Models를 참조하세요.
...
주어진 텍스트 "prompt"가 주어졌을 때, 이 시스템이 본질적으로 하는 일은 시스템이 알고 있는 모든 "vocabulary" (단어들의 목록 - 단어의 부분 또는 토큰)에 대한 확률 분포를 계산하는 것입니다. Vocabulary는 사람 설계자가 시스템에 부여합니다. 예를 들어 GPT-3에는 약 50,000개 토큰의 vocabulary가 있습니다. Source
